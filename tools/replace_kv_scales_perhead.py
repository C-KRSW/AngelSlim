...
def load_json_scales(json_path: str, tp_size: int, num_kv_heads: int) -> dict:
    ...
    pattern = re.compile(
        r"^(model\.layers\.\d+\.self_attn\.[kv]_cache)\.head_(\d+)\.scale$"
    )
...
def main():
    ...
    print(f"  num_kv_heads={args.num_kv_heads} "
          f"(legacy-fallback tp_size={args.tp_size})")
    new_scales = load_json_scales(args.json, args.tp_size, args.num_kv_heads)
    print(f"  Loaded {len(new_scales)} layer-slot entries "
    ...
    if missing:
        print(f"  NOTE: {len(missing)} keys in original file were NOT updated "
