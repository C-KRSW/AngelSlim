...
def parse_args():
    ...
    parser.add_argument("--ptq-data-path", type=str, required=True,
    ...
    parser.add_argument("--skip-weight-loading", action="store_true",
                        help="Use dummy weights for fast debug mode.")
    ...
    parser.add_argument("--distributed-executor-backend", type=str,
                        default="ray", choices=["ray", "mp"],
                        help="Distributed executor backend (default: ray).")

    parser.add_argument("--num-cols", type=int, default=64,
                        help="Number of leading P columns to analyse "
                             "(default: 64).")
    parser.add_argument("--q-block-size", type=int, default=2048,
                        help="Q-token block size for memory-bounded P "
                             "computation (default: 2048).")
    parser.add_argument("--no-per-head-detail", action="store_true",
...
def main():
    ...
    for layer_key, info in p_results.items():
        ...
        if head_sink_ratios:
            ...
            summary[layer_key]["head_sink_ratios_mean"] = (
                sum(head_sink_ratios) / len(head_sink_ratios)
            )
    ...
    for li in show_idx:
        ...
        m0 = col_means[0]
        m_tail = (
            sum(col_means[K:]) / max(len(col_means) - K, 1)
            if len(col_means) > K else 0.0
        )
        print(
    ...
    if layer_names:
        ...
        for j in range(n_show):
            ...
            print(
                f"  {j:>4d}  {col_means[j]:12.3e}  {col_maxes[j]:12.3e}  "
                f"{g:14.3f}"
            )
