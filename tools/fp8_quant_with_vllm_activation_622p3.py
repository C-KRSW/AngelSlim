...
def create_quantized_param(
    ...
    param_value = param.float().reshape(
        -1, math.ceil(rows / block_size_m), block_size_m, math.ceil(cols // block_size_n), block_size_n
    ).permute(0, 1, 3, 2, 4)
    ...
    with safe_open(os.path.join(bf16_path, file_name), framework="pt", device=f"cuda:{rank}") as f:
        ...
        for weight_name in f.keys():
            ...
            if any(weight_name.endswith(suffix) for suffix in SUFFIX_TO_QUANT):
                ...
                #                                              bits=4)  # create_quantized_param(weight, block_size)
                ...
                if block_size[0] == -1 and block_size[1] == -1:  # tensor-wise
                    ...
                    if ("mlp.experts" in weight_name or "mlp.gate" in weight_name or "mlp.up" in weight_name) and "down" not in weight_name:
                        prefix_name = '.'.join(name_parts[:-2])
                        act_vllm_name = prefix_name + ".gate_up_proj"
                    elif "q_proj" in weight_name or "k_proj" in weight_name or "v_proj" in weight_name:
                        prefix_name = '.'.join(name_parts[:-2])
                    ...
                    input_scale = max(abs(ac_json_data[act_vllm_name]["min"]), abs(ac_json_data[act_vllm_name]["max"])) / torch.finfo(
...
def worker(i, file_names, bf16_path, fp8_path, block_size, return_dict, ac_json_data):
    ...
    for file_name in tqdm(file_names, desc=f"Worker {i}"):
        index = process_safetensor(i % world_size, file_name, bf16_path, fp8_path, block_size, ac_json_data)
...
def main(bf16_path, fp8_path, block_size, ac_json_data):
    ...
    for i in range(args.num_workers):
        p = mp.Process(target=worker, args=(i, file_subsets[i], bf16_path, fp8_path, block_size, return_dict, ac_json_data))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
    ...
    for scale_name, stats in ac_json_data.items():
        ...
        if isinstance(min_val, list):
            ...
            per_head_scales = [
                max(abs(mn), abs(mx)) / fp8_max
                for mn, mx in zip(min_val, max_val)
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bf16_hf_path", type=str,
                        default='/apdcephfs_gy7/share_303171455/linchuanxie/new_hy3_vllm/HY3.0_A20B_bus0212_aisearch_full_exp3_spv2_HF')
    parser.add_argument("--input_vllm_ac_json_path", type=str,
                        default='/apdcephfs_gy7/share_303171455/linchuanxie/new_hy3_vllm/HY3.0_A20B_bus0212_aisearch_full_exp3_spv2_HF_fp8')
    parser.add_argument("--output_fp8_hf_path", type=str,
                        default='/apdcephfs_gy7/share_303171455/linchuanxie/new_hy3_vllm/HY3.0_A20B_bus0212_aisearch_full_exp3_spv2_HF_fp8')
    ...
    with open(os.path.join(args.input_vllm_ac_json_path,"activation_stats.json"), 'r', encoding='utf8') as fp:
        ac_json_data = json.load(fp)


    if os.path.isfile(os.path.join(args.input_vllm_ac_json_path,"moe_expert_stats.json")):
        with open(os.path.join(args.input_vllm_ac_json_path, "moe_expert_stats.json"), 'r', encoding='utf8') as fp:
