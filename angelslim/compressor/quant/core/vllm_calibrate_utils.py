...
def setup_activation_hooks(model, kv_granularity="per-tensor"):
    ...
    if kv_granularity == "none":
        ...
        print(f"---------Found {len(kvcache_layers)} kv cache layers to monitor (per-tensor)---------")
    ...
    if moe_layers:
        ...
        if not hasattr(draft_model, "_moe_activation_stats"):
            ...
            for name, layer in moe_layers.items():
                ...
                if hasattr(layer, "w13_weight") and layer.w13_weight is not None:
                    layer.w13_weight._vllm_layer_name = name
                    layer.w13_weight._moe_activation_stats_of_model = draft_model._moe_activation_stats
    ...
    * MSE values are accumulated into a GPU tensor; only a single
    ...
    if flat.is_cuda:
        ...
        for i in range(num_steps):
            ...
            scale = base_scale * m
            q_fp8 = (
                (flat / scale).clamp(fp8_min, fp8_max)
        ...
        for i in range(num_steps):
            m = math.exp(log_min + (log_max - log_min) * i / (num_steps - 1))
            scale = base_scale * m
            q_fp8 = (flat / scale).clamp(fp8_min, fp8_max).to(torch.float8_e4m3fn).to(torch.float32)
...
def _search_best_multiplier(
    ...
    return _search_best_multiplier_flat(flat, base_scale, min_multiplier, max_multiplier, num_steps)
...
class KVScaleSearcher:
    ...
    def __call__(self, model):
        ...
        if not kv_values:
            print("[KVScaleSearcher] WARNING: No kv values collected. "
        ...
        for layer_name, tensors_dict in kv_values.items():
            for kv_slot, tensors in tensors_dict.items():
                ...
                if stats_key not in self.activation_stats:
                    print(f"[KVScaleSearcher] WARNING: {stats_key} not found in "
        ...
        if use_gpu:
            print(f"[KVScaleSearcher] Running grid search on {search_device} "
                  f"({len(work_items)} work items, {self.num_steps} steps each)")
        else:
            print(f"[KVScaleSearcher] Running grid search on CPU "
        ...
        if use_gpu:
            for key, flat_cpu, base_scale in work_items:
                ...
                print(f"[KVScaleSearcher] {stats_key}: best_multiplier={best_m:.4f} "
                      f"(base_scale={bs:.6f})")
        else:
...
class KVCachePerHeadHook:
    ...
    def __init__(self, layer_name: str, kcache_stats: dict, vcache_stats: dict,
...
def _all_gather_stats_perhead(stats_dict, stats_type="statistics",
                              num_kv_heads_total: int | None = None):
    """
    ...
    **Idempotent**: uses a ``_gathered`` flag on ``stats_dict`` to ensure
    ...
    for name, stats in stats_dict.items():
        if name.startswith("__gathered_"):
            continue
        t = stats["min"]
        if isinstance(t, torch.Tensor) and t.ndim == 1 and t.numel() >= 1 and not (
            t.numel() == 1 and torch.isinf(t).all()
        ):
    ...
    if num_kv_heads_total is None or num_kv_heads_total <= 0:
        ...
        replication = max(1, world_size // num_kv_heads_total) \
            if num_kv_heads_total > 0 and world_size % num_kv_heads_total == 0 \
            else 1

    # Decide role & layout for this rank.
    role, heads_per_rank, global_head_offset, _ = \
        _compute_perhead_layout(rank, world_size, num_kv_heads_total)
    ...
    for name, stats in stats_dict.items():
        ...
        for key in ["min", "max"]:
            ...
            full = torch.full(
                (num_kv_heads_total,), neutral, dtype=torch.float32, device="cuda"
            )
            if have_data:
                t = stats[key]
...
class KVScaleSearcherPerHead:
    ...
    def __call__(self, model):
        ...
        if not kv_values:
            print("[KVScaleSearcherPerHead] WARNING: No per-head kv values collected. "
        ...
        role, heads_per_rank, global_head_offset, replication = \
            _compute_perhead_layout(rank, world_size, num_kv_heads_total)
        ...
        for layer_name, tensors_dict in kv_values.items():
            for kv_slot, tensors in tensors_dict.items():
                ...
                stats_key = f"{layer_name}.{kv_slot}_cache"
                if stats_key not in self.activation_stats:
                    print(f"[KVScaleSearcherPerHead] WARNING: {stats_key} not found in "
                          f"activation_stats, skipping.")
                    continue
                if not tensors:
        ...
        print(f"[KVScaleSearcherPerHead] rank={rank}/{world_size}, role={role}, "
        ...
        for key, head_dict in multipliers.items():
            ...
            mn = min(mults)
            mx = max(mults)
            print(f"[KVScaleSearcherPerHead] {key}: "
    ...
    def __call__(self, module, input, output):
        ...
        with torch.no_grad():
            ...
            if N_scales > 0 and stats["sum_sq"] is None:
                stats["sum_sq"] = torch.zeros(
                    H_q, N_scales, dtype=torch.float32, device=device
                )
                # Per-head sum of P^2 (same scope as sum_sq), used as the
                # denominator when computing NMSE.
                stats["p_norm_sq"] = torch.zeros(
                    H_q, dtype=torch.float32, device=device
                )
            ...
            if self.collect_dist and stats.get("dist_hist") is None:
                ...
                stats["dist_hist"] = torch.zeros(
                    H_q, num_bins, dtype=torch.float32, device=device
                )
                stats["dist_sum"] = torch.zeros(
                    H_q, dtype=torch.float32, device=device
                )
                stats["dist_sum_sq"] = torch.zeros(
                    H_q, dtype=torch.float32, device=device
                )
                stats["dist_max"] = torch.zeros(
                    H_q, dtype=torch.float32, device=device
                )
            ...
            if n_rep > 1:
                ...
                k_3d = k_3d.unsqueeze(2).expand(-1, -1, n_rep, -1).reshape(
                    T_k, H_q, D
                )
                ...
                for q_start in range(0, S, Bq):
                    ...
                    if N_scales > 0:
                        ...
                        if K_skip > 0 and K_skip < S:
                            skip_mask = torch.ones(
                                1, 1, S, dtype=torch.float32, device=device
                            )
                        ...
                        for i, s in enumerate(self.scale_list):
                            ...
                            P_q = (P_block / s).clamp_(-fp8_max, fp8_max).to(
                                torch.float8_e4m3fn
                            ).to(torch.float32).mul_(s)
                        ...
                        if K_skip > 0:
                            ...
                            ks = min(K_skip, S)
                            j_idx = torch.arange(
                                ks, device=device, dtype=torch.float32
                            )
                            skipped_rows = (
                                float(q_end) - torch.clamp(
                                    j_idx, min=float(q_start)
                                )
                            ).clamp_(min=0.0).sum().item()
                    ...
                    if N_first > 0:
                        ...
                        col_idx = torch.arange(
                            ncols, device=device, dtype=torch.float32
                        )
                        valid = (
                            float(q_end) - torch.clamp(col_idx, min=float(q_start))
                        ).clamp_(min=0.0)
                    ...
                    if do_hist:
                        ...
                        # vectorised and fast.
                        edges = torch.tensor(
                            self.HIST_EDGES, dtype=torch.float32, device=device
                        )
                        bucket_idx = torch.bucketize(
                            p_flat, edges, right=False
                        )
                        bucket_idx = (bucket_idx - 1).clamp_(
                            min=0, max=len(self.HIST_EDGES) - 2
                        )
...
def get_p_matrix_scale_stats(model):
    ...
    for layer_name, stats in model._p_matrix_stats.items():
        ...
        if has_mse:
            ...
            global_p_norm_sq = _allgather_heads(
                p_norm_sq_cpu.unsqueeze(1)
            ).squeeze(1)
        ...
        if has_dist:
            ...
            global_dist_sum = _allgather_heads(dist_sum_cpu.unsqueeze(1)).squeeze(1)       # (H_q_total,)
        ...
        if has_mse:
            ...
            mse_skip = int(getattr(
                model, "_p_matrix_mse_skip_first_cols", 0
            ))
        ...
        if has_dist:
            ...
            if dist_count_global > 0:
                ...
                # Var = E[x^2] - E[x]^2
                var = (global_dist_sum_sq / dist_count_global) - (global_dist_sum / dist_count_global) ** 2
        ...
        if has_first_cols:
            ...
            tail_mean = (
                float(mean_per_col_layer[best_K:].mean().item())
                if best_K < N_local else 0.0
            )
            if tail_mean > 0:
            ...
            keep_per_head = bool(getattr(
                model, "_p_matrix_first_cols_per_head_detail", True
            ))
            if keep_per_head:
