...
def parse_args():
    ...
    parser.add_argument(
        "--skip-weight-loading", action="store_true",
    ...
    # Distributed
    parser.add_argument(
        "--distributed-executor-backend", type=str, default="ray",
        choices=["ray", "mp"],
    ...
    parser.add_argument(
        "--q-block-size", type=int, default=256,
    ...
    parser.add_argument(
        "--no-collect-p-dist", action="store_true",
    ...
    parser.add_argument(
        "--mse-skip-first-cols", type=int, default=2,
...
def main():
    ...
    if p_dist_per_head:
        ...
        if hist_edges_ref is not None:
            edge_strs = [f"{e:.0e}" if e > 0 and e < 1e-2 else f"{e:.3g}"
                         for e in hist_edges_ref]
            bin_labels = [
                f"[{edge_strs[i]},{edge_strs[i+1]})"
                for i in range(len(edge_strs) - 1)
            ]
