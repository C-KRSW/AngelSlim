...
try:
    ...
    _angelslim_logger.debug(
        "[AngelSlim] Trying to import from tools_path: %s", _tools_path
    )
    ...
    _angelslim_logger.info(
        "[AngelSlim] Successfully imported collect_fused_moe_internal_stats"
    )
...
class TritonExperts(LoRAExpertsMixin, mk.FusedMoEExpertsModular):
    ...
    def apply(
        ...
        if (
            _layer_name is None
            and "_vllm_layer_name" not in _ANGELSLIM_WARNED_ATTRS
        ):
            logger.warning(
        ...
        _moe_activation_stats = getattr(
            w1, "_moe_activation_stats_of_model", None
        )
        if (
