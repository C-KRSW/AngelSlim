...
def main():
    ...
    if args.enable_mtp:
        ...
        print(f"  MTP calibration: setting max_tokens={calibration_max_tokens} "
    ...
    if args.kv_granularity == "per-head":
        ...
        if not stats_list_ph or stats_list_ph[0] is None:
            ...
            if args.search_kv_scale:
                ...
                print(f"\nRunning {len(search_prompts_ph)} forward passes for per-head KV-value collection...")
                llm.generate(
                ...
                kv_multipliers_perhead = get_kv_scale_search_results_perhead(search_results_list_ph)

                llm.apply_model(remove_kvcache_perhead_value_hooks)

                multipliers_ph_path = os.path.join(args.output_dir, "kv_scale_multipliers_per_head.json")
                ...
                tuned_ph_path = os.path.join(args.output_dir, "kv_cache_tuned_scales_per_head.json")
